...
const validateCSS = (css: string): CSSError[] => {
  ...
  lines.forEach((line, lineIndex) => {
    ...
    if (inRuleSet && !trimmedLine.includes(':') && !trimmedLine.includes('}') && !trimmedLine.includes('{')) {
    ...
    if (inRuleSet && trimmedLine.includes(':') && !trimmedLine.endsWith(';') && !trimmedLine.endsWith('{') && !trimmedLine.endsWith('}')) {
      errors.push({
        line: lineIndex + 1,
        column: line.length,
    ...
    if (trimmedLine.includes(':')) {
      ...
      if (property && !property.match(/^[a-z-]+$/) && !property.startsWith('-')) {
        errors.push({
          line: lineIndex + 1,
          column: 1,
...
export function CSSEditor({ 
  ...
  const [activeTab, setActiveTab] = useState<'editor' | 'templates' | 'hints'>('editor');
  ...
  const insertTemplate = useCallback((template: typeof CSS_TEMPLATES[0]) => {
          ...
          <Card key={index} className="p-4">
            <div className="space-y-2">
              <div className="flex items-center gap-2">
                ...
                <code className="font-semibold text-purple-600">{hint.property}</code>
              </div>
          ...
          <div className="space-y-4">
            <div className="page-background p-6 rounded-lg">
              ...
              <p className="text-gray-600 mb-4">
                This is a sample description to show how your custom CSS affects the page styling.
              </p>
