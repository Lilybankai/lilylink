...
export function HeaderBlock({ value, onChange, className = '' }: HeaderBlockProps) {
  ...
  const handleTypeChange = useCallback((type: HeaderBlockConfig['type']) => {
    let newConfig: HeaderBlockConfig = {
      ...value,
      type,
  ...
  const handleMediaChange = useCallback((mediaConfig: Partial<HeaderBlockConfig['media']>) => {
    onChange({
      ...value,
      media: {
        url: value.media?.url || '',
  ...
  const handleOverlayChange = useCallback((overlayConfig: Partial<HeaderBlockConfig['media']['overlay']>) => {
  ...
  const handleContentChange = useCallback((contentConfig: Partial<HeaderBlockConfig['content']>) => {
    onChange({
      ...value,
      content: {
    ...
    return (
      <div className="relative">
        ...
        >
          ...
          {/* Content overlay */}
          {value.type !== 'none' && (
            <div className={`absolute inset-0 flex items-${value.content?.titlePosition || 'center'} justify-center p-8`}>
              <div className="text-center">
                {value.content?.showTitle && (
                  <h1 
                    ...
                    style={{ 
                      ...
                      textShadow: value.content.textShadow ? '0 2px 4px rgba(0,0,0,0.5)' : 'none'
                    }}
                  >
                    Your Name
                  </h1>
                ...
                {value.content?.showDescription && (
                  <p 
                    ...
                    style={{ 
                      color: value.content.textColor,
                      textShadow: value.content.textShadow ? '0 1px 2px rgba(0,0,0,0.5)' : 'none'
                    }}
                  >
  ...
  return (
    <div className={`space-y-6 ${className}`}>
      ...
      <div>
        ...
        <div className="grid grid-cols-4 gap-2">
          {[
            ...
            { type: 'gradient' as const, icon: PaintBrushIcon, label: 'Gradient' },
      ...
      {value.type !== 'none' && (
        <div>
          <h3 className="text-sm font-medium text-gray-700 mb-3">Header Height</h3>
      ...
      <AnimatePresence mode="wait">
        {value.type === 'gradient' && (
          <motion.div
            ...
            className="space-y-4"
          >
            <h3 className="text-sm font-medium text-gray-700">Gradient Background</h3>
            ...
            <div>
              ...
              <div className="grid grid-cols-2 gap-3">
                {PRESET_GRADIENTS.map((gradient) => (
                  <button
                    key={gradient.name}
                    onClick={() => onChange({ ...value, gradient: { value: gradient.value } })}
                    className={`
            ...
            <div>
              ...
              <GradientPicker
                ...
                onChange={(gradientValue) => onChange({ ...value, gradient: { value: gradientValue } })}
              />
            </div>
          </motion.div>
        )}
        ...
        {(value.type === 'image' || value.type === 'video') && (
          ...
          >
                ...
                <div className="space-y-3">
                  ...
                  <div>
                    <p className="text-sm text-gray-600 mb-2">
                      Drag & drop {value.type === 'image' ? 'an image' : 'a video'}, or{' '}
                      <button
                        onClick={() => value.type === 'image' ? fileInputRef.current?.click() : videoInputRef.current?.click()}
                    ...
                    <p className="text-xs text-gray-500">
                      {value.type === 'image' ? 'PNG, JPG, GIF up to 10MB' : 'MP4, WebM up to 50MB'}
                    </p>
                  </div>
                </div>
              )}
            ...
            {/* URL Input */}
            <div>
              <p className="text-xs text-gray-600 mb-2">Or enter {value.type} URL</p>
              <Input
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    ...
                    <select
                      ...
                      onChange={(e) => handleMediaChange({ position: e.target.value })}
                  ...
                  <div>
                    ...
                    <select
                      ...
                      onChange={(e) => handleMediaChange({ size: e.target.value })}
                      className="w-full px-3 py-2 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent"
                    >
      ...
      {value.type !== 'none' && (
        <Card className="p-4">
          <div className="space-y-4">
            <h4 className="text-sm font-medium text-gray-700">Content Overlay</h4>
            ...
            <div className="space-y-3">
              <label className="flex items-center gap-2">
                <input
                  ...
                  onChange={(e) => handleContentChange({ showTitle: e.target.checked })}
              ...
              <label className="flex items-center gap-2">
                <input
                  ...
                  onChange={(e) => handleContentChange({ showDescription: e.target.checked })}
                  className="rounded border-gray-300 text-purple-600 focus:ring-purple-500"
                />
            ...
            {/* Text Position */}
            <div>
              <label className="block text-xs text-gray-600 mb-1">Text Position</label>
              <select
                value={value.content?.titlePosition || 'center'}
                onChange={(e) => handleContentChange({ titlePosition: e.target.value as any })}
                className="w-full px-3 py-2 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent"
              >
            ...
            <label className="flex items-center gap-2">
              <input
                ...
                onChange={(e) => handleContentChange({ textShadow: e.target.checked })}
                className="rounded border-gray-300 text-purple-600 focus:ring-purple-500"
              />
        ...
        <Card className="p-4">
          <div className="space-y-3">
            <div className="flex items-center justify-between">
              <h4 className="text-sm font-medium text-gray-700">Background Overlay</h4>
              <label className="flex items-center gap-2">
                <input
                  type="checkbox"
              ...
              <div className="space-y-3">
                ...
                <div>
                  <label className="block text-xs text-gray-600 mb-1">
                    Opacity: {Math.round((value.media.overlay.opacity || 0.3) * 100)}%
                  </label>
                  <input
                    type="range"
                    min="0"
